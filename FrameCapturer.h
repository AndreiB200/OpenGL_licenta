...
public:
    FrameCapturer(int width, int height)
        : m_width(width), m_height(height), m_bufferSize(width* height * 4)
    {
        // Alocăm PBO-urile: 2 pentru Color, 2 pentru Depth/Second Attachment
        ...
        // Pre-alocăm spațiul pentru vectorii CPU
        m_colorBuffer.resize(m_bufferSize);
        m_depthBuffer.resize(m_bufferSize);
    }
    ...
    void CaptureAndProcess(DroneSim& droneSim, Streamer& streamer) {
        ...
        if (colorPtr) {
            std::copy(colorPtr, colorPtr + m_bufferSize, m_colorBuffer.begin());
            glUnmapBuffer(GL_PIXEL_PACK_BUFFER);
        }

        glBindBuffer(GL_PIXEL_PACK_BUFFER, m_pboDepth[readIdx]);
        uint8_t* depthPtr = static_cast<uint8_t*>(glMapBuffer(GL_PIXEL_PACK_BUFFER, GL_READ_ONLY));
        if (depthPtr) {
            std::copy(depthPtr, depthPtr + m_bufferSize, m_depthBuffer.begin());
            glUnmapBuffer(GL_PIXEL_PACK_BUFFER);
        }
